...
int GarbageCandsIDX, GarbageRepsIDX, PruneGarbageSize, Stride, MappingIdx = 0;
int timeIDX;
...
void External_Voting(void*p, char* MissHit,int direction,int size){
	...
	if(direction){
		...
		do {
			...
			if(accsesTime >= THRESHOLD) MissHit[i++]++;			
			else i++;
			p = LNEXT(p);
		} while (p != (void *) pp);
	}
...
int reduction_iterative(Struct addresses) {
    ...
    while (top >= 0) {
        ...
        if ((cur.N_c < W) || (cur.N_R == 0)) {
            if (cur.N_R) collectReps(cur.Representatives);
            if (cur.N_c) collectCands(cur.candidates);
            continue;
        }
        ...
        if (cur.N_R == 1) {
            ...
            if (cnt >= 10) {
                ...
                if ((cur.N_c < (3*W/2)) && checkEviction(cur.candidates, cur.Representatives, cur.candidates)) {			//We can allow eviction sets with more then W! (cur.N_c < (3*W/2))
                    //printf("Cought: %d\n",cur.N_c);
					collectEvictionSet(cur);
                    size = -1;
                    result++;
            ...
            if ((prev == cur.Representatives) && (size == cur.N_c)) cnt++;
            else {
        ...
        Rtail = LNEXT(NEXTPTR(Rhead));
		middleHead = Rhead;
		for (i = 0; i < ceil((float)cur.N_R / 2); i++) middleHead = LNEXT(middleHead);
...
void PruneInfo(void* head,void* tail,char *MissHit,int NumExp,int size,void* mapping_head){
	...
	for(int iterations=0;iterations<NumExp;iterations++){
		...
		if(mapping_head != NULL) Prime(mapping_head,FW);
		
		External_Voting(tail,MissHit,!FW,size);
	}
	for(int j=0;j<size;j++){
		if(MissHit[j] == NumExp) MissHit[j] = 1;
		else MissHit[j] = 0;
	}
}	
...
Struct Prune(Struct addresses){
	...
	for(int i=0;i<2;i++){
		...
		for(int j=0;j<addresses.N_c;j++) if(MissHit1[j] != MissHit2[j]) MissHit1[j] = 0;  		//if(MissHit1[j] != MissHit2[j]) MissHit1[j] = 0; 	
		...
		if(probe_args.N2 != 0){
			if(PruneGarbage == NULL) PruneGarbage = probe_args.sec;
			else mergeLists(PruneGarbage, probe_args.sec);
			//flush(PruneGarbage);
		}
...
Struct BuildTrees(Struct addresses){
	...
	else improveFactor = 100;
	if(addresses.N_c==0 || addresses.N_R==0) return addresses;
	...
	do{		
		...
		if(timeIDX >= BufferSize) break;		//For ploting figure;
...
Struct map_LLC(float LLC_Cover, Struct addresses){
	...
	while(((float)MappingIdx/S)*100 < LLC_Cover){
		...
		if((addresses.N_R == 0) || (addresses.N_c == 0)) break;
	}	
	addresses.N_c = N_c;
	addresses.N_c = N_R;	
	return addresses;
